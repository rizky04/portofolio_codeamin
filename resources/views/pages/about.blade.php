        ...
        <div class="mb-12">
            ...
            <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-4 anim-stagger-parent">
                ...
                <div class="skill-card anim-stagger-child opacity-0 glass-card rounded-xl p-4 flex flex-col items-center gap-3">
                    @if($skill->icon && str_starts_with($skill->icon, 'fa-'))
                        <div class="w-12 h-12 rounded-xl {{ $cfg['color'] }} flex items-center justify-center">
                            <i class="{{ $skill->icon }} text-2xl"></i>
                        </div>
                    @elseif($skill->icon && str_starts_with($skill->icon, 'http'))
                        <img src="{{ $skill->icon }}" alt="{{ $skill->name }}" class="w-10 h-10 object-contain">
                    @elseif($skill->icon)
                        <span class="text-3xl">{{ $skill->icon }}</span>
                    @else
                        <div class="w-12 h-12 rounded-xl {{ $cfg['color'] }} flex items-center justify-center font-mono font-bold text-sm">
